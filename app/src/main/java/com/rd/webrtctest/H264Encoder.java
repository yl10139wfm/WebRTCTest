...
public class H264Encoder {
    ...
    private static final String path = Environment.getExternalStorageDirectory().getAbsolutePath() + "/rtc.h264";
                ...
                if (input != null) {
                    try {
                        ...
                        while (outputBufferIndex >= 0) {
                            //Log.i("H264Encoder", "Get H264 Buffer Success! flag = "+bufferInfo.flags+",pts = "+bufferInfo.presentationTimeUs+"");
    ...
    private void NV21ToNV12(byte[] nv21, byte[] nv12, int width, int height) {
        ...
        for (j = 0; j < framesize / 2; j += 2) {
            if (framesize + j - 1 < framesize) {

                nv12[framesize + j - 1] = nv21[j + framesize];
            }
        }
        for (j = 0; j < framesize / 2; j += 2) {
            if (framesize + j < framesize) {
